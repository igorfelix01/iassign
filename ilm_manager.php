...
function prepare_secure_access ($ilmid, $fileid, $userid) {
  ...
  //D echo "ilm_manager.php: prepare_secure_access($ilmid,$fileid,$userid): <br/>";
  //print_r($md_file); echo "<br/>";
  $filename = $md_file->get_filename(); // $md_file is instanceof 'class file_storage' from './lib/filestorage/file_storage.php'
  //D echo "md_file->file_record->filename=" . $filename . "<br/>";
  ...
  //D echo "ilm_manager.php: prepare_secure_access($ilmid,$fileid,$userid): filename=".$security['filename']."; " .$security['content']. " " .$security['token']. " " .$security['secure_id']. "<br/>";
  return $security;
  }
...
//d echo "<br/><br/>ilm_manager.php: returnurl=$returnurl<br/>";
...
//d $fileid = optional_param('fileid', 1, PARAM_INT); echo "ilm_manager.php: ilmid=$ilmid, fileid=$fileid, id=$id, user.id=$USER->id<br/>"; //exit;
...
// Enter here whenever : teacher is viewing any iLM content (preview), here is called by the iLM do not verify credentials.
if ($action == 'get') {
